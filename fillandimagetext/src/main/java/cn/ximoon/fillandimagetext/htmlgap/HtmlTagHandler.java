...
public class HtmlTagHandler implements Html.TagHandler  {
    ...
    public HtmlTagHandler(TextView tv, RelativeLayout rl,Context context,ClickController clickController){
        this.tv = tv;
...
//            }
            ...
            if (DEBUG) {
                int index = -1;
                int lastIndex =0;
                String str = "[__Fill.Replace__]";
                String line = "__________";
                outputSpan(output, str, line, lastIndex);
            }
        }
    }
    private int  outputSpan(Editable output, String str, String line, int lastIndex) {
        int index = -1;
        while (-1 != (index = output.toString().indexOf("[__Fill.Replace__]", lastIndex))) {
            lastIndex = index;
            indexs.add(lastIndex);//将下标添加到集合
            //将下标替换
            output.replace(lastIndex, lastIndex + str.length(), line);
